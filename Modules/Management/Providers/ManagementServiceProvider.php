...
class ManagementServiceProvider extends ServiceProvider
{
     ...
     */
    protected $moduleNameLower = 'management';
    ...
    public function boot()
    {
        ...
        Module::macro("isEnable",function ($moduleName){
            return in_array($moduleName,array_keys(Module::allEnabled()));
        });

        Module::macro("canDisable",function ($module){
            $moduleInfo = new Json($module->getPath()."\Module.json");
            $status = $moduleInfo->get("canDisable");
            return (is_null($status) || $status==true) ? true : false;
        });
